...
int main(int argc, const char **argv)
{
	if(argc == 1)
	{
		...
		cout<<"usage: "<<argv[0]<< " compare <gtf-file1> <gtf-file2> [options]"<<endl;
	...
	if(string(argv[1]) == "compare")
	{
		...
		g1.build_all_transcripts(argv[2]);
		g2.build_all_transcripts(argv[3]);

		g1.compare_small_difference(g2);
	}
